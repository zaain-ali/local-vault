...
const vectorsPath = "../../../../spec/protocol-test-vectors.json"
...
func buildVectors(t *testing.T) *vectorFile {
	...
	v := &vectorFile{Description: "LocalVault protocol v4 test vectors (spec/protocol.md). Binary fields are standard base64 with padding; *_hex fields are lower-hex; aad/info/messages are UTF-8 strings. Generated by apps/cli/internal/lvcrypto (LV_WRITE_VECTORS=1)."}
...
func TestWriteVectors(t *testing.T) {
	if os.Getenv("LV_WRITE_VECTORS") != "1" {
		t.Skip("set LV_WRITE_VECTORS=1 to regenerate " + vectorsPath)
	}
	data, err := json.MarshalIndent(buildVectors(t), "", "  ")
	if err != nil {
		t.Fatal(err)
	}
